...
def spirebuilder(id,scale,X,Y,Z):
    scale = scale * 0.1
    objID = id
    StringID = int(objID / 1000000000000)
    stupidLongString = ""
  #Actual XML here
...
<Position x= "''' + str(X) + '''" y="''' + str(Y + 6 * (scale)) + '''" z="''' + str(Z) + '''"/>
...
<Position x= "''' + str(X) + '''" y="''' + str(Y + 13.5 * (scale)) + '''" z="''' + str(Z) + '''"/>
...
<Scale x="''' + str(3 * scale) +'''" y="''' + str(2 * scale) +'''" z="''' + str(3 * scale) +'''" />
<Data>
<Color key="bmt-colour">
<R>0.3254902</R>
<G>0.4196078</G>
